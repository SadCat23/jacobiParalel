...
void printResult(const int size, const long double* x, string name)
{
	ofstream out;
	out.open(name+".txt");
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		int sizeArray = 0;            // <---- put here matrix size 
		...
		calculateLengthsAndOffsets(size, sizeArray, lengths, lengths_a, offsets, offsets_a);
		...
		if (rank == 0)
		{
			cout << "Number of processes: " << size << " Size: " << sizeArray << " Time: " << finish - start << endl;
			printResult(sizeArray, x, nameOut);
		}
